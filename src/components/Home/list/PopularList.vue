    ...
    <v-flex xs12 sm11 md10 lg9 xl8>
      <v-container fluid style="margin-top:-2vw;">
        <v-layout row wrap px-5 mt-4>
          <!-- 농활카드하나씩 -->
          <v-flex tag="a" v-for="(item,index) in popularList" :key="index" xs6 sm4 md4 lg4 xl4 id="card_gutter" @click="goNonghwalDetail(item.nhIdx)">
          ...
          <v-btn color="white" v-if="isEndPopul==0||isEndPopul==[]"
